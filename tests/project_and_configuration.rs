...
/// can reach it: that module is `pub(crate)` and this binary links the crate's
/// public surface alone. Nothing is added — a directory under the system's
/// temporary location, named for the process and a counter, is unique without
/// coordination and needs no dependency.
#[derive(Debug)]
struct Sandbox {
...
impl Sandbox {
    /// Makes a temporary directory with no `.tpl` folder anywhere above it.
    fn new() -> Self {
        let name = format!(
            "tpl-project-{}-{}",
            std::process::id(),
    ...
    /// A path inside the sandbox, which need not exist.
    ...
    /// A project at `relative`, whose `.tpl/.cfg` holds `configuration` at the
    /// mode `FR-PROJ-011` requires.
    fn project_at(&self, relative: &str, configuration: &str) -> PathBuf {
    ...
    /// A project at the root of the sandbox.
    fn project(&self, configuration: &str) -> PathBuf {
