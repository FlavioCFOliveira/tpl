...
///
/// A differential run compares two invocations made in two directories, so the
/// suite needs two of these at once and needs each to be free of anything an
/// upward walk could find.
#[derive(Debug)]
pub struct Sandbox {
...
impl Sandbox {
    ...
    pub fn project(&self, configuration: &str) -> PathBuf {
        ...
        let cfg = self.write(".tpl/.cfg", configuration);
